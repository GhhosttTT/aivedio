...
async def produce_video(project_id: int):
    """
    提交视频生产任务
    ...
    try:
        ...
        # 创建生产任务
        task = task_orchestrator.create_production_task(project_id)
        
        # 更新项目状态
        project_manager.update_project(project_id, status="producing")
        
        logger.info(f"生产任务创建成功: task_id={task.task_id}, project_id={project_id}")
