...
This module is the walking skeleton for the short-drama export workflow:
upload Chinese source video -> remove subtitles/watermarks -> ASR -> translate
subtitles -> burn subtitles -> moderation -> multilingual outputs.
"""
...
class LocalizationPipeline:
    ...
    def save_source_video(self, project_id: int, upload: UploadFile) -> SourceVideo:
        ...
        logger.info("源片保存成功: project_id={}, source_video_id={}", project_id, source_video.id)
        return source_video
    ...
    def create_job(
        ...
        if not source_video:
            raise ValueError(f"源片不存在: {source_video_id}")
        ...
        logger.info("出海译制任务创建成功: job_id={}, languages={}", job.id, languages)
        return job
    ...
    def get_job(self, job_id: int) -> LocalizationJob:
        ...
        if not job:
            raise ValueError(f"译制任务不存在: {job_id}")
        return job

    def run_cleaning(self, job: LocalizationJob) -> str:
        raise NotImplementedError(
            f"未接入去字幕/去水印后端: {settings.SUBTITLE_REMOVAL_BACKEND}"
        )

    def run_asr(self, job: LocalizationJob) -> str:
        raise NotImplementedError(f"未接入 ASR 后端: {settings.ASR_BACKEND}")

    def run_translation(self, job: LocalizationJob) -> str:
        raise NotImplementedError(f"未接入翻译后端: {settings.TRANSLATION_BACKEND}")

    def run_rendering(self, job: LocalizationJob) -> str:
        raise NotImplementedError("未接入多语种字幕压制")

    def run_moderation(self, job: LocalizationJob) -> str:
        raise NotImplementedError(f"未接入审核后端: {settings.MODERATION_BACKEND}")
