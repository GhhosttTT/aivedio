...
def test_production_readiness_blocks_non_mobile_format(setup, monkeypatch):
    ...
    monkeypatch.setattr("src.services.production_readiness.settings.GENERATION_HEIGHT", 768)
...
def test_production_readiness_warns_when_sample_video_gates_fail(setup):
    ...
    assert "sample_validation_video_review_not_passed" in codes
...
def test_production_readiness_warns_when_sample_manual_clip_review_is_missing(setup):
    client, _, path = setup
    validation_dir = path / "storage" / "validation"
    validation_dir.mkdir(parents=True, exist_ok=True)
    (validation_dir / "validation_summary.json").write_text(json.dumps({
        ...
        "checks": {
            ...
            "manual_review_covers_rendered_cases": True,
            "manual_review_missing_case_ids": [],
            "manual_clip_review_present": False,
            "manual_clip_score": None,
        ...
        "action_items": ["Add manual clip review."],
    }), encoding="utf-8")
    ...
    payload = response.json()
    validation = payload["checks"]["sample_validation"]
    assert validation["checks"]["manual_clip_review_passed"] is False
    assert any(item["code"] == "sample_validation_manual_clip_review_not_passed" for item in payload["warnings"])


def test_production_readiness_warns_when_sample_repair_queue_is_not_empty(setup):
    client, _, path = setup
    validation_dir = path / "storage" / "validation"
    validation_dir.mkdir(parents=True, exist_ok=True)
    (validation_dir / "validation_summary.json").write_text(json.dumps({
        ...
        "repair_queue": [{"action": "regenerate_character_identity"}],
        "action_items": ["Resolve all repair_queue actions."],
    }), encoding="utf-8")
...
def test_production_readiness_warns_when_sample_render_profile_is_not_production(setup):
    ...
    (validation_dir / "validation_summary.json").write_text(json.dumps({
        ...
        "checks": {
            ...
            "manual_review_missing_case_ids": [],
            "manual_review_passed": True,
            "render_profile": {"quality_mode": "normal", "optimization_mode": "balanced"},
            "render_profile_passed": False,
            "render_workflow_parameters_passed": False,
        },
        "action_items": [],
    }), encoding="utf-8")
    ...
    payload = response.json()
    validation = payload["checks"]["sample_validation"]
    assert validation["checks"]["render_profile_passed"] is False
    assert any(item["code"] == "sample_validation_render_profile_not_production" for item in payload["warnings"])


def test_production_readiness_warns_when_sample_workflow_parameters_are_missing(setup):
    client, _, path = setup
    validation_dir = path / "storage" / "validation"
    validation_dir.mkdir(parents=True, exist_ok=True)
    (validation_dir / "validation_summary.json").write_text(json.dumps({
    ...
    payload = response.json()
    validation = payload["checks"]["sample_validation"]
    assert validation["checks"]["render_workflow_parameters_passed"] is False
    assert any(item["code"] == "sample_validation_workflow_parameters_missing" for item in payload["warnings"])
