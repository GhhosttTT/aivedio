...
class TestProjectManager:
    ...
    def test_create_project_success(self, project_manager, test_user, temp_storage):
        """测试创建项目（成功）"""
    ...
    def test_create_project_empty_name(self, project_manager, test_user):
        """测试创建项目（名称为空）"""
        with pytest.raises(ValueError, match="项目名称不能为空"):
            project_manager.create_project(name="", user_id=test_user.id)
    
    def test_create_project_name_too_long(self, project_manager, test_user):
        """测试创建项目（名称过长）"""
        long_name = "A" * 101
        with pytest.raises(ValueError, match="项目名称不能超过100个字符"):
            project_manager.create_project(name=long_name, user_id=test_user.id)
    
    def test_create_project_invalid_characters(self, project_manager, test_user):
        """测试创建项目（包含非法字符）"""
        with pytest.raises(ValueError, match="项目名称只能包含"):
            project_manager.create_project(name="测试@#$%项目", user_id=test_user.id)
    
    def test_get_project_success(self, project_manager, test_user, temp_storage):
        """测试获取项目（成功）"""
        # 先创建项目
        created_project = project_manager.create_project(name="测试项目", user_id=test_user.id)
    ...
    def test_update_project_success(self, project_manager, test_user, temp_storage):
        """测试更新项目（成功）"""
        # 先创建项目
        project = project_manager.create_project(name="原始名称", user_id=test_user.id)
    ...
    def test_update_project_status(self, project_manager, test_user, temp_storage):
        """测试更新项目状态"""
        project = project_manager.create_project(name="测试项目", user_id=test_user.id)
    ...
    def test_delete_project_success(self, project_manager, test_user, temp_storage):
        """测试删除项目（成功）"""
        # 先创建项目
        project = project_manager.create_project(name="待删除项目", user_id=test_user.id)
    ...
    def test_list_projects_all(self, project_manager, test_user, temp_storage):
        """测试列出所有项目"""
        # 创建多个项目
        project_manager.create_project(name="项目1", user_id=test_user.id)
        project_manager.create_project(name="项目2", user_id=test_user.id)
        project_manager.create_project(name="项目3", user_id=test_user.id)
    ...
    def test_list_projects_with_status_filter(self, project_manager, test_user, temp_storage):
        """测试列出项目（状态过滤）"""
        # 创建不同状态的项目
        p1 = project_manager.create_project(name="草稿项目", user_id=test_user.id)
        p2 = project_manager.create_project(name="完成项目", user_id=test_user.id)
    ...
    def test_list_projects_with_pagination(self, project_manager, test_user, temp_storage):
        """测试列出项目（分页）"""
        # 创建多个项目
        for i in range(5):
            project_manager.create_project(name=f"项目{i+1}", user_id=test_user.id)
    ...
    def test_count_projects(self, project_manager, test_user, temp_storage):
        """测试统计项目数量"""
        # 创建项目
        project_manager.create_project(name="项目1", user_id=test_user.id)
        project_manager.create_project(name="项目2", user_id=test_user.id)
    ...
    def test_add_character(self, project_manager, test_user, temp_storage):
        """测试添加角色"""
        # 先创建项目
        project = project_manager.create_project(name="测试项目", user_id=test_user.id)
        ...
        assert character.visual_description == "高大威猛"
    ...
    def test_add_character_empty_name(self, project_manager, test_user, temp_storage):
        """测试添加角色（名称为空）"""
        project = project_manager.create_project(name="测试项目", user_id=test_user.id)
    ...
    def test_add_scene(self, project_manager, test_user, temp_storage):
        """测试添加分镜"""
        # 先创建项目
        project = project_manager.create_project(name="测试项目", user_id=test_user.id)
        ...
        scene = project_manager.add_scene(
            project_id=project.id,
            scene_number=1,
            visual_description="阳光明媚的早晨",
            dialogue="你好，世界！",
            character_name="主角",
            image_prompt="sunny morning, bright sky"
        )
        
        assert scene.id is not None
        assert scene.project_id == project.id
        assert scene.scene_number == 1
        assert scene.visual_description == "阳光明媚的早晨"
    ...
    def test_add_scene_empty_description(self, project_manager, test_user, temp_storage):
        """测试添加分镜（描述为空）"""
        project = project_manager.create_project(name="测试项目", user_id=test_user.id)
    ...
    def test_get_project_scenes(self, project_manager, test_user, temp_storage):
        """测试获取项目分镜"""
        # 创建项目并添加分镜
        project = project_manager.create_project(name="测试项目", user_id=test_user.id)
    ...
    def test_get_project_characters(self, project_manager, test_user, temp_storage):
        """测试获取项目角色"""
        # 创建项目并添加角色
        project = project_manager.create_project(name="测试项目", user_id=test_user.id)
